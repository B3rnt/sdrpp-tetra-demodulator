...
/*
 * SDRTetra-compat tweaks:
 *  - PDISC detectie moet werken op "unpacked bits" én softbits (0x00/0xFF/..).
 *    Daarom altijd bit = (byte & 1) voor de eerste 3 bits.
 *  - Traffic filter maakbaar via compile-time define (soms worden control-achtige blokken
 *    gemarkeerd als traffic afhankelijk van chain/receiver).
 */
#ifndef MM_BYPASS_TRAFFIC_FILTER
#define MM_BYPASS_TRAFFIC_FILTER 1
#endif
...
static void try_decode_to_mle(struct tetra_mac_state *tms, struct msgb *msg)
{
    ...
    /* --- ADDED/IMPROVED (SDRTetra-style): log PDISC + ISSI/SSI context --- */
    if (l2len >= 3) {
        uint8_t pdisc;

        /*
         * SDRTetra-style: l3h is vaak 1-byte-per-bit (0/1) of softbits (0x00/0xFF/..).
         * Pak daarom altijd alleen de LSB van de eerste 3 bytes.
         */
        pdisc = (uint8_t)(((msg->l3h[0] & 1u) << 2) |
                          ((msg->l3h[1] & 1u) << 1) |
                          ((msg->l3h[2] & 1u) << 0));
