...
static void mm_log_result(uint32_t issi, uint16_t la, const struct t34_result *r)
{
    ...
    if (r->have_cck) {
        ...
        strncat(tail, tmp, 500 - (unsigned)strlen(tail));
    }

    if (r->have_itsi && r->itsi) {
        strncat(tail, " - ITSI attach", 500 - (unsigned)strlen(tail));
    } else if (r->have_roam && r->roam) {
        if (r->have_srv_rest && r->srv_rest)
            strncat(tail, " - Service restoration roaming location updating", 500 - (unsigned)strlen(tail));
        else
            strncat(tail, " - Roaming location updating", 500 - (unsigned)strlen(tail));
    }
